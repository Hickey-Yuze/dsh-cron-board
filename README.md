...
任务到点后新建**独立 DSH 会话**执行任务 Prompt，会话终态（turn/end）即任务终态；执行简讯经 dsh-im 机器人推送，完整结果落盘可回看。
...
- **真实执行**：`agentLoop.create` 新会话 → 可钉住工作区/agent 预设/权限档 → followup 发送任务 Prompt → `turn/end` 终态结算（超时 cancel 兜底）。
- **权限确认门**：有效权限高于默认档（read-only）的任务，首次 cron 触发前须在看板人工确认一次；权限、Prompt、预设、工作区任一变更自动重新武装；未确认时 cron 跳过并滚动。
- **推送**：终态简讯（✅/❌ + 任务名 + 耗时 + 结果路径）经 `dshIm` 服务直推 dsh-im 机器人；dsh-im 未部署时自动降级 HTTP 回退（默认 `127.0.0.1:3080`），仍不通则标记「未送达」，看板可手动**补推**。结果始终落盘。
...
> 路径含空格时 dsh CLI 会拆参，请先建无空格 junction 再 add：
> `cmd /c mklink /J C:\Users\<你>\.dsh\cron-board-src "<含空格的项目路径>"`
> 然后 `dsh plugin --profile web add link:C:\Users\<你>\.dsh\cron-board-src`
...
- 数据独立存放于 `$DSH_HOME/cron-board/`（`ledger.json` + `results/<taskId>/<execId>.md`）；**不读写**宿主既有 `.dsh\cron`，也不触碰其他插件的 `.dsh\task-board`。
