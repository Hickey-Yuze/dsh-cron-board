...
export class ResultStore {
  ...
  async write(task: TaskRow, exec: Execution, body: string): Promise<string> {
  ...
  async read(taskId: string, execId: string, maxBytes = 262144): Promise<string | undefined> {
    if (!/^[A-Za-z0-9_-]+$/.test(taskId) || !/^[A-Za-z0-9_-]+$/.test(execId)) return undefined;
