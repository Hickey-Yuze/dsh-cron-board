...
interface ConnectionLike {
  rpc: {
    handle(channel: string, handler: (endpoint: string, payload: unknown) => unknown): unknown;
  };
}
...
export function registerRpc(ctx: import('@deepseek-ai/cordis').Context, deps: RpcDeps): void {
  let attached = false;

  const attach = (): void => {
    if (attached) return;
    const conn = ctx.get('connection') as unknown as ConnectionLike | undefined;
  ...
  async function handler(endpoint: string, payload: unknown): Promise<unknown> {
    switch (endpoint) {
      ...
      case 'cron-board/meta': {
        ...
        try {
          const reg = ctx.get('workspaceRegistry') as unknown as { list?(): { id?: string; title?: string; path?: string }[] } | undefined;
