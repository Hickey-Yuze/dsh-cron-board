...
export class Scheduler {
  ...
  constructor(
    private readonly ledger: {
      snapshot: { tasks: TaskRow[] };
      mutate<T>(fn: (doc: { tasks: TaskRow[] }) => T | Promise<T>): Promise<unknown>;
    },
  ...
  async tick(): Promise<void> {
    ...
    try {
      ...
      for (const task of this.ledger.snapshot.tasks) {
        ...
        if (now.getTime() < current.getTime()) continue; // 未到期
        // 到期：先决定动作，再统一滚动
        const missed = now.getTime() - current.getTime() > this.deps.tickMs + 90_000;
        let skip: string | null = null;
        if (missed) skip = 'missed-trigger-skipped';
        else if (this.runner.isRunning(task.id)) skip = 'task-running-skipped';
        else if (gateDecision(task, this.deps.defaultPermission) === 'awaiting-confirmation') skip = 'awaiting-confirmation';
        else fire.push(task);
