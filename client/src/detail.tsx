...
export function DetailModal(props: {
      ...
      const r = await props.rpc('cron-board/task-upsert', {
        task: {
          ...
          pinned: {
            ...
            permission: permission === '' ? undefined : (permission as 'read-only' | 'workspace-write' | 'danger-full-access'),
          },
  ...
  async function action(fn: () => Promise<{ ok: true; value: unknown } | { ok: false; error: { message: string } }>, okMsg: string): Promise<void> {
    setBusy(true);
    setErr(null);
    setNote(null);
    try {
  ...
  return createElement(
    'div',
    { className: 'dsh-cb-modal-mask', onClick: (e: { target: unknown; currentTarget: unknown }) => {
    ...
    createElement(
      ...
      createElement(
        ...
        createElement('h3', { className: 'dsh-cb-title', style: { margin: 0, fontSize: 14 } }, task ? t('dl.editTitle') : t('dl.newTitle')),
        createElement('div', { className: 'dsh-cb-spacer' }),
        task ? createElement(Badge, { tone: task.enabled ? 'accent' : 'default' }, task.enabled ? t('act.enable') : t('act.disable')) : null,
      ...
      createElement(
          ...
          ? createElement(
              ...
              task.needsConfirm && !task.confirmed
                ? createElement(
                    'div',
                    { className: 'dsh-cb-row' },
                    createElement(Btn, { disabled: busy, onClick: () => void action(() => props.rpc('cron-board/task-confirm', { id: task.id }), t('dl.confirmed')) }, t('act.confirm')),
                  )
                : null,
            )
          : null,
        ...
        createElement(Field, { label: t('f.cron'), hint: cronPreview },
          createElement(TextInput, { value: cron, onChange: setCron }),
          !cronOk ? createElement('span', { className: 'dsh-cb-hint', style: { color: 'var(--dsh-cb-err, #d5372f)' } }, 'cron: invalid') : null,
        ),
        createElement(
          'div',
          { className: 'dsh-cb-row' },
          createElement(
            ...
            createElement(Field, { label: t('f.workspace') },
          ...
          createElement(
            'div',
            { style: { flex: 1, minWidth: 180 } },
            createElement(Field, { label: t('f.preset') },
              createElement(Select, {
                value: presetId,
        ...
        createElement(
          ...
          createElement(
            'div',
            { style: { flex: 1, minWidth: 180 } },
            createElement(Field, { label: t('f.permission'), hint: perm !== undefined && perm !== 'read-only' ? t('gate.body') : undefined },
              createElement(Select, {
                value: permission,
          ...
          createElement(
            'div',
            { style: { flex: 1, minWidth: 180 } },
            createElement(Field, { label: t('f.push') },
        ...
        createElement(
          ...
          task
            ? createElement(Btn, { disabled: busy || task.running, onClick: () => void action(() => props.rpc('cron-board/task-run', { id: task.id }), t('dl.runQueued')) }, t('act.run'))
            : null,
          task
            ? createElement(Btn, { disabled: busy, onClick: () => void action(() => props.rpc('cron-board/push-test', { id: task.id }), t('dl.pushTested')) }, t('act.testPush'))
            : null,
          task
            ? createElement(Btn, { disabled: enabled === task.enabled, onClick: () => void action(() => props.rpc('cron-board/task-toggle', { id: task.id, enabled }), enabled ? t('act.enable') : t('act.disable')) }, enabled ? t('act.enable') : t('act.disable'))
            : null,
                ...
                : task.executions.map((exec) =>
                    createElement(
                      ...
                      createElement(
                        ...
                        createElement(Dot, { tone: exec.status === 'running' ? 'run' : exec.status === 'success' ? 'ok' : exec.status === 'failed' ? 'err' : 'warn' }),
                        createElement('strong', { style: { fontSize: 12 } }, t(`st.${exec.status}`)),
                        createElement(Badge, {}, t(`exec.trigger.${exec.trigger}`)),
                        exec.push
                          ? createElement(Badge, { tone: exec.push.state === 'sent' ? 'ok' : exec.push.state === 'failed' ? 'err' : 'default' }, `${t('exec.push')}: ${t(`push.${exec.push.state}`)}`)
                          : null,
                        createElement('div', { className: 'dsh-cb-row-right' }),
                        exec.push?.state === 'failed'
                          ? createElement(Btn, { kind: 'ghost', disabled: busy, onClick: () => void action(() => props.rpc('cron-board/push-retry', { taskId: task.id, execId: exec.id }), t('dl.pushTested')) }, t('act.retryPush'))
                          : null,
                      ...
                      createElement(
                        ...
                        exec.durationMs !== undefined ? createElement('span', null, `${t('exec.duration')} ${fmtDur(exec.durationMs)}`) : null,
