...
.dsh-cb-modal { width: min(760px, calc(100vw - 48px)); max-height: calc(100vh - 64px); display: flex; flex-direction: column; overflow: hidden; background: var(--dsh-cb-card); border: 1px solid var(--dsh-cb-border); border-radius: 14px; box-shadow: var(--dsh-cb-shadow); color: var(--dsh-cb-text); }
.dsh-cb-modal-head { display: flex; align-items: center; gap: 10px; padding: 14px 18px; border-bottom: 1px solid var(--dsh-cb-border); flex-shrink: 0; background: var(--dsh-cb-card); }
.dsh-cb-modal-body { padding: 14px 18px 18px; display: flex; flex-direction: column; gap: 12px; overflow-y: auto; min-height: 0; }
