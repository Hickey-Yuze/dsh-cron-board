...
export class Pusher {
    ...
    opts;
    log;
    constructor(resolveDshIm, opts, log) {
        this.resolveDshIm = resolveDshIm;
        this.opts = opts;
        this.log = log;
    }
    channelView() {
    ...
    async listBots() {
        ...
        if (!svc?.listBots)
            return [];
        try {
            const raw = (await svc.listBots());
            return (Array.isArray(raw) ? raw : [])
                .filter((b) => typeof b?.botId === 'string')
                .map((b) => ({ botId: b.botId, channel: typeof b.channel === 'string' ? b.channel : 'unknown' }));
        }
        catch (err) {
    ...
    async listTargets(botId) {
        ...
        if (!svc?.listTargets)
            return [];
        try {
            const raw = (await svc.listTargets(botId));
            return (Array.isArray(raw) ? raw : [])
                .filter((t) => typeof t?.targetId === 'string')
                .map((t) => ({ targetId: t.targetId, name: t.name, kind: t.kind }));
        }
        catch (err) {
    ...
    async send(botId, targetId, text) {
        ...
        for (let i = 1; i <= max; i++) {
            ...
            if (i < max)
                await new Promise((res) => setTimeout(res, 1000 * 2 ** (i - 1)));
        }
    ...
    briefFor(input) {
        ...
        if (input.ok) {
            ...
            if (dur)
                lines.push(lang === 'zh' ? `耗时：${dur}` : `Duration: ${dur}`);
        }
        else {
            ...
            if (dur)
                lines.push(lang === 'zh' ? `已运行：${dur}` : `Ran for: ${dur}`);
        }
        if (input.resultPath) {
