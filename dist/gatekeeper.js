...
export function confirmFingerprint(t) {
...
/** 任务有效权限档：钉住优先，否则部署默认。 */
export function effectivePermission(t, defaultPermission) {
    return t.pinned.permission ?? defaultPermission;
}
/** 高于默认权限档的任务需要一次性人工确认（默认档 = read-only 时，非 read-only 都要确认）。 */
export function needsConfirm(t, defaultPermission) {
...
export function gateDecision(t, defaultPermission) {
    if (!needsConfirm(t, defaultPermission))
        return 'allow';
