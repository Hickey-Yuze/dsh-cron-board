...
function parseField(raw, name, min, max, isDow = false) {
    ...
    for (const term of terms) {
        ...
        if (slash >= 0) {
            ...
            if (!Number.isInteger(step) || step < 1)
                throw new Error(`cron: 字段 ${name} 步长不合法: "${term}"`);
        }
        let lo, hi;
        if (body === '*') {
            lo = min;
            ...
            if (parts.length !== 2)
                throw new Error(`cron: 字段 ${name} 范围不合法: "${term}"`);
            lo = Number(parts[0]);
            hi = Number(parts[1]);
        }
        else {
        ...
        if (!Number.isInteger(lo) || !Number.isInteger(hi))
            throw new Error(`cron: 字段 ${name} 数值不合法: "${term}"`);
        if (isDow) {
            if (lo === 7)
                lo = 0;
            if (hi === 7)
    ...
    if (values.size === 0)
        throw new Error(`cron: 字段 ${name} 无有效值: "${raw}"`);
    return { values, wild };
}
export function parseCron(expr) {
    const parts = expr.trim().split(/\s+/);
    if (parts.length !== 5)
        throw new Error(`cron: 需要 5 段（分 时 日 月 周），收到 ${parts.length} 段: "${expr}"`);
...
function dayMatches(c, d) {
    if (c.domWild && c.dowWild)
        return true;
    if (c.domWild)
        return c.dow.has(d.getDay());
    if (c.dowWild)
        return c.dom.has(d.getDate());
 ...
 */
export function cronNextRun(expr, from) {
...
export function describeCron(expr, lang) {
    let parts;
    try {
